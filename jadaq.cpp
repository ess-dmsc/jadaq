...
int main() {
    ...
    digitizer.openUSB(0);
    digitizer.reset();
    caen::Digitizer::BoardInfo boardInfo = digitizer.getInfo();
    std::cout << "I am talking to a " << boardInfo.modelName() << ", serial: " << boardInfo.serialNumber() << std::endl;
    std::cout << "ADCbits " << boardInfo.ADCbits() << std::endl;
    ...
    for (int i = 0; i < 1; ++i)
    {
        ...
        //uint32_t nEvents = digitizer.getNumEvents(data);
