...
_CAEN_DGTZ_DPP_QDC_Event_t *gEventsGrp[MAX_GROUPS] = {NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL};
...
static CAEN_DGTZ_ErrorCode V1740DPP_QDC_MallocDPPEvents(int handle, void **events, uint32_t *allocatedSize)
{
    ...
    *allocatedSize = size;
    ...
    uint32_t helperSize = MAX_CHANNELS*MAX_EVENT_QUEUE_DEPTH*sizeof(_CAEN_DGTZ_DPP_QDC_Event_t);
    for(i=0; i<MAX_GROUPS; i++) {
        if (gEventsGrp[i] == NULL) {
            if ((gEventsGrp[i] = (_CAEN_DGTZ_DPP_QDC_Event_t *)malloc(helperSize)) == NULL)
                return CAEN_DGTZ_OutOfMemory;
            /* Initialize contents to zero to avoid unitialized use */
...
static CAEN_DGTZ_ErrorCode V1740DPP_QDC_MallocDPPWaveforms(int handle, void** waveforms, uint32_t* allocatedSize)
{
     ...
     * for consistency. */
    uint32_t size = sizeof(_CAEN_DGTZ_DPP_QDC_Waveforms_t) + 2*samples*sizeof(uint16_t) + 4*samples*sizeof(uint8_t);
    _CAEN_DGTZ_DPP_QDC_Waveforms_t* buffer = (_CAEN_DGTZ_DPP_QDC_Waveforms_t*)malloc(size);
    if (buffer == NULL)
        return CAEN_DGTZ_OutOfMemory;
    /* Initialize contents to zero to avoid unitialized use */
...
int _CAEN_DGTZ_DPP_QDC_DecodeDPPAggregate(int handle, uint32_t *data, _CAEN_DGTZ_DPP_QDC_Event_t *Events, int *NumEvents) 
{
    uint32_t i = 0, size = 0, evsize = 0, nev = 0, et = 0, eq = 0, ew = 0, ee = 0, pnt = 0;
...
int _CAEN_DGTZ_DPP_QDC_GetDPPEvents(int handle, char *buffer, uint32_t BufferSize, _CAEN_DGTZ_DPP_QDC_Event_t **Events, uint32_t *NumEvents)
{
    ...
    while(pnt < (uint32_t)(BufferSize/4) - 1) { 
        ...
        for(grp=0; grp<grpmax; grp++) {
            if (!(grpmask & (1<<grp)))
                continue;
            if (_CAEN_DGTZ_DPP_QDC_DecodeDPPAggregate(handle, buffer32+pnt, Events[grp]+index[grp], &nevgrp))
                return CAEN_DGTZ_InvalidEvent;
            index[grp] += nevgrp;
...
static CAEN_DGTZ_ErrorCode V1740DPP_QDC_GetDPPEvents(int handle, char *buffer, uint32_t BufferSize, void **Events, uint32_t *NumEvents)
{
    ...
    /* the for loop distributes events belonging to groups into single channel event arrays */
    for(i=0; i<MAX_GROUPS; i++) {
...
static CAEN_DGTZ_ErrorCode V1740DPP_QDC_DecodeDPPWaveforms(int handle, _CAEN_DGTZ_DPP_QDC_Event_t* event, _CAEN_DGTZ_DPP_QDC_Waveforms_t *gWaveforms) {
