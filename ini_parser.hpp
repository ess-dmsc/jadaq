    ...
    if (it->second.empty()) {
      ...
      for (auto &subdiv : it->second) {
        if (!subdiv.second.empty()) {
          BOOST_PROPERTY_TREE_THROW(
              ini_parser_error("ptree is too deep", "", 0));
        }
        stream << it->first << Ch('[') << subdiv.first << Ch(']') << Ch('=')
               << subdiv.second.template get_value<std::basic_string<Ch>>()
               << Ch('\n');
      }
    }
  }
